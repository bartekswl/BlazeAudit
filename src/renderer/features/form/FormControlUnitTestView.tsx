    ...
    return (
      <td className={tdCls}>
        <span className="cut-check-cell cut-check-cell--readonly">
          <FormCheckGlyph checked={variant === choice} className="cut-check" />
        </span>
      </td>
    );
  }
...
export function FormControlUnitTestView({
  ...
  const choosableIds = CONTROL_UNIT_TEST_ROWS.filter((row) => row.kind !== 'firmware').map(
    (row) => row.id,
  );

  const applyColumn = (variant: ControlUnitTestChoice) => {
    ...
    for (const id of choosableIds) {
      nextData = setControlUnitTestChoice(nextData, id, next);
    }
    onChange?.(nextData);
  };
